label utami_intro:
    ...
    n "Хм-м~ Ну, *Я* в восторге! Давно не была у Ютами. Она всегда делает из меня такую красотку~"
    ...
    ut "Меня зовут Ютами! Офигеть как приятно познакомиться~!"
    ...
    "Ютами жестом приглашает Нору сесть, пока ты и Зара неловко отходите в сторону."
    ...
    "Ютами тихо хихикает, пока Зара выглядит явно недовольной этой колкостью."
    scene utamiintro (19) with dissolve
    ...
    "Хотя Ютами говорит это игриво, Нора, кажется, напрягается от этого предложения."
    ...
    "Нора громко сплетничает, пока Ютами подстригает ее волосы словно проводит священный ритуал."
    ...
    scene black with fade
    "После сушки феном, спрея с блестками и мини-фотосессии от Ютами, Нора выглядит так, будто только что сошла с обложки журнала."
    ...
    menu:
        ...
        "Ты выглядишь... точно так же.":
            ...
            "Ютами и Нора сдерживают смешок, прежде чем вернуть серьезный вид."
            n "Оу, [mcname]."
    ...
    "Ютами улыбается тебе, когда ты направляешься к выходу."
    ...
    if _in_replay:
        ...
        "{i} Теперь вы можете навещать Ютами по средам после уроков.{/i}"
        $ intro_utami += 1
        $ Utami.affection += 1
        stop music fadeout 1.0
        $ Utami.quest = "Навести Ютами после уроков в среду."
...
label utami_hair_intro:
    ...
    mc "(Ютами сказала, что я могу заскочить после уроков... Почему бы и нет. Все лучше, чем заниматься абсолютно ничем.)"
    ...
    "Ютами напевает себе под нос, сидя за столом и время от времени бросая на тебя взгляды через плечо."
    ...
    ut "Так-с~ Салон снова выглядит круто благодаря тебе, красавчик~"
    ...
    if _in_replay:
        ...
        $ Utami.quest = "Навести Ютами в салоне"
...
label city_noonutami:
    ...
    "Гудение фенов и болтовня манят тебя к салону Ютами."
    scene black with fade

label utami_menu:
    play music "audio/Music/utami.mp3" fadein 1.0
    scene UtamiWeekend with dissolve
    "Ютами занята протиркой кресла, но она с ухмылкой смотрит на тебя, когда ты заходишь."
...
label mall_eveninghangutami:
    ...
    "Ютами напевает под какую-то веселую музыку, пока вы уборкой занимаетесь бок о бок."
...
label mall_eveningwaitutami:
    "Ты чиллишь в салоне, пока Ютами заканчивает закрывать заведение."
    jump weekday_evening
...
label utami_beach_one:
    ...
    "Ютами тихо смеется и смотрит через плечо."
    ...
    "Внезапно Ютами встает со своего места."
    ut "Угх-х-х~ Сегодня на улице {b}слишком{/b} жарко! Я буквально умираю здесь."

    "Ютами драматично обмахивается одной рукой, отклоняясь назад, отчего ее цветное худи-кроп слегка задирается."
    ...
    "Она наклоняется через стойку к тебе, ее яркие волосы падают вперед."
    ...
    ut "Я бы с супер-охотой сгоняла на пляж с тобой. Что скажешь, красавчик? Поехали со мной?"
    ...
    ut "Это не странно, если ты сам не делаешь это странным, красавчик."
    ...
    "Ютами выдает гордую ухмылочку, высунув язычок."
    ...
    scene black with dissolve
    "Вы оба быстро заканчиваете последние приготовления к уборке. Ютами переворачивает табличку «Закрыто» и запечатывает входную дверь с веселым мурлыканьем."
    ...
    ut "Я могу выглядеть фифой и пустышкой, но не позволяй радужным волосам и блесткам обмануть тебя, малыш."
    ...
    ut "Вот это настрой! Погнали ловить волны, красачик~"

    scene black with fade
    "После быстрой остановки у Ютами дома, чтобы переодеться и забрать плавки, вы наконец приезжаете на пляж."
    ...
    "Ютами выглядит абсолютно в своей стихии в обтягивающем ярком бикини, которое оставляет очень мало места воображению."
    ...
    scene black with dissolve
    "Вы оба заходите в прохладную воду, пока она не доходит примерно до пояса. Ютами встает рядом с тобой."
    ...
    scene surfbeach1 (44) with dissolve
    "Быстро приближается еще одна волна, Ютами подбирает момент."
    ut "Готов? Вот идет одна."

    "Ютами дает тебе легкий толчок в поясницу для нужного момента."
    ...
    scene surfbeach1 (44) with dissolve
    "Ютами остается рядом, давая постоянные советы, пока ты делаешь еще несколько попыток."
    ...
    ut "Продолжай! Я не позволю тебе сдаться, пока ты не проедешься хотя бы на одной волне нормально. Приказ тренера Ютами."
    ...
    ut "Но я обещаю, что сделаю из тебя серфера, красавчик."
    ...
    stop music fadeout 1.0
    scene black with dissolve
    "Вы переодеваетесь, и Ютами везет тебя обратно к школе."
    ...
    if _in_replay:
        ...
        play sound "audio/Sound/affection.mp3" volume 0.5
        "{i}Привязанность Ютами увеличена.{/i}"
        $ Utami.quest = "Навести Ютами снова в ее салоне."
...
label utami_beach_two:
    ...
    "Ютами за стойкой, мурлычет себе под нос, расставляя кое-какие баночки."
    ...
    "Ютами бросает взгляд в окно на яркий солнечный свет, затем возвращается к тебе с лукавым огоньком в глазах."
    ...
    ut "Тонкость — это скучно. Так что скажешь, красавчик? Пляж, раунд два?"

    mc "Ладно, ладно. Я в теме."
    scene surfbeach2 (13) with dissolve
    ut "Е-е-е! Вот это я понимаю подход."

    "Ютами радостно хлопает в ладоши, ее яркие волосы подпрыгивают."
    ...
    "Тебе удается встать на нескольких небольших волнах и даже проехать на одной приличное расстояние. Ютами подбадривает тебя каждый раз."
    ...
    "Ютами звонко смеется, гребя рядом с тобой на своей доске в том же самом крошечном цветном бикини."
    ...
    "Твоя доска сильно заваливается. Ты теряешь равновесие, падаешь вбок и врезаешься прямо в Ютами."
    ...
    "Оказавшись за скалами и скрывшись из виду, Ютами издает раздраженный вздох, все еще прикрываясь."
    ...
    "Ютами замечает это почти мгновенно."
    ...
    "Не дожидаясь окончательного ответа, Ютами запускает руки под твои плавки и высвобождает твой твердеющий член."
    ...
    "Ютами начинает двигать бюстом вверх и вниз, скользя по твоему члену между сисек плавными, размеренными движениями."
    ...
    "Ютами ускоряет темп, сжимая сиськи еще сильнее и время от времени облизывая головку твоего члена, когда она выскакивает между ними."

    ut "Ах-х~ Ты становишься еще тверже… Тебе приятно, красавчик?"
    ...
    "Ютами издает довольный тихий стон, выдавливая последние капли своей грудью."
    ...
    if _in_replay:
        ...
        $ utamibeachbj += 1
        play sound "audio/Sound/affection.mp3" volume 0.5
        "{i}Привязанность и похоть Ютами увеличены.{/i}"
