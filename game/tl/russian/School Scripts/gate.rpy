...
screen gate_screen(g=gate_girls):
    add "gate_bg"
    for i in g.clicks:
        button:
            focus_mask True pos i.pos
            add i.image
            at gate_button_animation
            action i.action
...
transform gate_button_animation:
    on idle:
        ease .1 yoffset 0 additive 0
    on hover:
        ease .1 yoffset -10 additive .2

default gate_bg = class_click("gate_bg", "gate_bg", None, (0, 0))
default gate_kyra = class_click("Kyra", "gate_kyra", Jump("gate_kyra"), (450, 300))


default gate_girls = class_handler(
    [
        gate_bg,
        gate_kyra, 
    ]
...
label gate_example:
    scene gate_bg
    call screen gate_screen

label gate_kyra:
    if intro_kyra == 0:
        jump kyra_intro
    elif intro_kyra == 1 and firstfight == 1 and kyraalleyevent == 0:
        jump kyra_alley_event
    elif kyraalleyevent == 1 and kyragangevent == 0:
        jump kyra_recruitment_advice
    else:
